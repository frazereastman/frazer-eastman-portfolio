<script setup>
	...
	const getFabURL = computed(() => {
		return fabIconDetails[props.icon].url;
	});
	const getFabIcon = computed(() => {
		return fabIconDetails[props.icon].icon;
	});
	const getFabHoverText = computed(() => {
		return fabIconDetails[props.icon].hoverText;
	});
</script>
